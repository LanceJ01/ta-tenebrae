...
std::string extract_direction(const std::string &input) {
    std::vector<std::string> direction_words = {"north", "south", "east",
                                                "west"};
...
class Room {
    public:
        ...
        void add_room_exit(const std::string &direction, Room *room) {
            room_exits[direction] = room;
        }
        ...
        bool can_unlock(const std::string &direction,
        ...
        bool remove_item(const std::string &item_name) {
            auto it = std::remove_if(items.begin(), items.end(), [&](Item &i) {
                return to_lowercase(i.item_name) == item_name;
            });
            if (it != items.end()) {
    ...
    if (room_current->get_exit(direction)) {
        if (room_current->is_exit_locked(direction)) {
            if (room_current->can_unlock(direction, player.player_inventory)) {
                ...
                std::cout
                    << "The door is locked. Maybe there's a key nearby...\n\n";
                return;
            }
        }
...
std::map<std::string, Item> item_library = {
    {"rusted knife",
     Item("rusted knife",
          "A slightly dull rusted knife. Could be useful later...")},
...
void start_new_game() {
    ...
    Room room_start_east(
        "You look down at the dirty ragged bed... it seems just a minute ago "
        "you were having the worst nightmare...\n",
        "You feel under the bed and find a rusted knife.\n");
    room_start_east.revealed_item_name = "rusted knife";
    Room room_start_west("You stare blankly at the wall...\n");
    Room room_prison_hallway(
        "You step through the cell door into a dark hallway. The hallway "
    ...
    while (true) {
        ...
        if (player_action.find("search") != std::string::npos ||
            ...
            if (room_current->has_been_searched_by_player() &&
                ...
                Item *item =
                    room_current->find_item(room_current->revealed_item_name);
                if (item) {
                    player.add_to_inventory(*item);
                    room_current->remove_item(item->item_name);
                    room_current->revealed_item_name
                        .clear(); // prevent double-take
                } else {
